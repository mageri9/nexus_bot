...
def _build_docker_container(
    key: str, cfg: Dict[str, Any], transport: Transport, redis_client: Optional[Redis] = None
) -> Resource:
    return DockerContainer(key, transport, cfg["container_name"])


def _build_project_storage(
    key: str, cfg: Dict[str, Any], transport: Transport, redis_client: Optional[Redis] = None
) -> Resource:
    return ProjectStorageResource(key, transport, cfg["path"])


def _build_host_disk(
    key: str, cfg: Dict[str, Any], transport: Transport, redis_client: Optional[Redis] = None
) -> Resource:
...
def _build_heartbeat(
    ...
    return ApplicationHeartbeat(
        key,
        cfg["project"],
        max_gap_seconds=cfg.get("max_gap_seconds", 30),
        redis_client=redis_client,
    )


RESOURCE_FACTORY: Dict[
    str, Callable[[str, Dict[str, Any], Transport, Optional[Redis]], Resource]
] = {
...
def build_resource(
    ...
    return factory(key, config, transport, redis_client)
